...
// Single task card — used in both mobile and tablet/desktop card view
...
// Bucket swim lane grouping with collapsible sections
  ...
  return (
    <div className="flex flex-col gap-6">
      {grouped.map(([bucket, bucketTasks]) => {
        ...
        return (
          <div key={bucket}>
            {/* Swim lane header — clickable to collapse */}
            <button
...
export default function Dashboard() {
  ...
  // Fixed: do NOT setSelectedTask(null) after save — let the modal manage its own close
  const handleSave = async (id: number, updates: Record<string, any>) => {
    ...
    await refetch();
    // Do NOT close modal here — modal controls its own close via onClose
  };
